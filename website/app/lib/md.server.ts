...
export async function processMarkdown(content: string, options?: ProcessorOptions) {
    ...
    if (!parseFrontMatter.test(content)) {
        console.error('Failed parsing front matter', content)
        const vfile = await processor.process(content)
        const html = vfile.value.toString()

        return { attributes: {}, raw: content, html }
    }
...
export async function loadPlugins() {
    const [{ visit, SKIP }] = await Promise.all([import('unist-util-visit'), import('escape-goat')])
    ...
    const remarkCodeBlocksShiki: InternalPlugin<UnistNode.Root, UnistNode.Root> = (_options) => {
        ...
        return async function transformer(tree: UnistNode.Root) {
            ...
            visit(tree, 'code', (node) => {
                if (!node.lang || !node.value || !langs.includes(node.lang)) {
                    return
                }
