...
import { MenuItem, menuData } from '~/config/menu.js'
...
export function DesktopButton({
  ...
  return (
    <Box
      ...
      py="5"
      className={
        index === activeIndex ? css({ outline: '1px', outlineColor: '2023-gray-light-ii', outlineStyle: 'solid' }) : ''
      }
    >
      <styled.button
        display="flex"
      ...
      {index === activeIndex ? (
        ...
        >
          ...
          <Box
            ...
            bg="2023-white-ii"
          >
            {item.subPages?.map((subPage, index) => <DropdownButton subPage={subPage} style={`child`} key={index} />)}
          </Box>
        </Box>
      ) : null}
    </Box>
