...
The DDD-family conference websites include a community voting system that helps select talks from submitted proposals. This document explains how the voting system works, including the underlying algorithms and ELO rating calculations. Any conference using the talk-voting feature gets this system unchanged from `ddd-core`.
...
Due to network latency and rapid voting, multiple votes for the same session can arrive out of order. The system uses **monotonic forward-only updates** in D1 to handle this safely:
...
async function updateSessionIndexSafely(
    ...
    // Only move forward — stale votes for earlier rounds are ignored.
...
D1 serialises writes within a single SQL statement, so the index-only-moves-forward invariant holds without explicit ETag/version comparison — a later vote for a lower index simply takes no action.
...
- **Votes are never lost** — even if the session-index update fails, the vote row is already written before we attempt the update
- **Graceful degradation** — session tracking may be slightly behind if a write fails, but remains consistent because the next vote re-reads `current_index` before moving it
- **No retry loop** — D1's per-statement serialisation makes the monotonic update safe without an explicit CAS retry; errors are logged and the request continues
