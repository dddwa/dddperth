...
The banner copy itself is **fork-owned and required** — `retired.notice` has no core default, so a fork that retires its app without writing a sentence gets a compile error rather than shipping one core wrote in its voice. Worth stating what happened ("we haven't made any updates for 2026") rather than calling the app out of date, broken or unsupported: the agenda, speaker and announcement endpoints are served live from here, so a retired build still shows current data and generally still works. Being a literal string, it won't advance on its own — a fork that stays retired needs to revisit the year it mentions.
