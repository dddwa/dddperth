 ...
 * MUST track the @playwright/test version in core/website/package.json —
 * a mismatch reintroduces exactly the drift this exists to remove.
 *
 * Usage:
...
const websiteDir = path.join(repoRoot, 'core', 'website')
...
if (!(await waitForPort(PORT, 120_000))) {
    console.error('[vr] dev server did not start within 120s — run `node core/website/e2e/start-dev-server.mjs` to see why')
    stopServer()
    process.exit(1)
}
